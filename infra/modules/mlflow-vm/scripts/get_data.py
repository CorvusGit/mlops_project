...
def plot_clean_stress_results(df, dates_json_session=""):
    ...
    g.map(sns.lineplot, "timestamp", "w_rps", marker="o", color="royalblue")
    
    g.set_axis_labels("Секунды теста", "w_rps")
    g.set_titles("Нагрузка: {col_name} RPS")
    
    plt.subplots_adjust(top=0.9)
    g.fig.suptitle('Динамика w_rps (без аномальных выбросов)', fontsize=16)
...
def s3_to_kafka(
    ...
    for current_rps in RPS_STEPS:
        ...
        while proc.is_alive():
            ...
            tt = int(datetime.now().timestamp()) - start_time
            results.append({
                'rps': current_rps,
                'timestamp': tt,
                'output_total': out_total,
                'w_rps': out_total/(tt+1)
            })
            print(f"[{current_rps} RPS] w_rps: {out_total/(tt+1)}")
    ...
    df_clean = df_perf[
        df_perf['w_rps'] <= df_perf.groupby('rps')['w_rps'].transform('mean') * 3
    ].copy()
